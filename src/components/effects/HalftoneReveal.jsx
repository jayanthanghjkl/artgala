...
void main() {
  ...
  // Removed fluid ripple/bulge effect
  vec3 sharp = sharpRGB(texture(tMap, clamp(coverUv(vUv), 0.0, 1.0)).rgb);

  // CHANGED THIS: Instead of mix(print, sharp, focus), we do mix(sharp, print, focus)
  // This means the background is the sharp clear image, and hovering reveals the halftone print!
...
const HalftoneReveal = ({
  ...
  useEffect(() => {
    ...
    const loop = now => {
      ...
      m.sx += (m.x - m.sx) * a;
      m.sy += (m.y - m.sy) * a;
      const ba = 1 - Math.exp(-dt / 0.05); // Changed from 0.18 to 0.05 for fast fade
