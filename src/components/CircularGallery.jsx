...
class Media {
  ...
  update(scroll, direction) {
    ...
    // No infinite wrap needed since we are driven by scroll progress
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    ...
    this.mediasImages = galleryItems; // Do not concat for infinite scroll
  ...
  setProgress(progress) {
    ...
    const maxScroll = this.medias[0].width * (this.medias.length - 1);
...
const CircularGallery = forwardRef(({
  ...
  useImperativeHandle(ref, () => ({
    setProgress: (p) => {
      if (appRef.current) appRef.current.setProgress(p);
    }
  }));
  useEffect(() => {
  ...
  return (
    <div
      className="w-full h-full overflow-hidden focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4"
      tabIndex={0}
      role="region"
