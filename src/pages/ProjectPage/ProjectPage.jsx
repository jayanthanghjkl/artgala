  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        start: 'top top',
        end: `+=${galleryProjects.length * 200}vh`,
        pin: true,
        scrub: true,
        snap: {
          snapTo: (predictedProgress) => {
            ...
            // Limit the snap target to a maximum of 1 index away from the active index
            let targetIndex = predictedIndex;
            setActiveIndex((currentIndex) => {
              if (predictedIndex > currentIndex + 1) targetIndex = currentIndex + 1;
              if (predictedIndex < currentIndex - 1) targetIndex = currentIndex - 1;
              return currentIndex; // Don't actually change state here, just read it safely
            });
            
            return targetIndex * step;
          },
          duration: 0.4,
          delay: 0.05, // Faster delay so it catches the fast scrolls quicker
          ease: "power1.inOut"
        },
